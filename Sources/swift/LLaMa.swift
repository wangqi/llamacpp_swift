...
public class LLaMa: LLMBase {
     ...
     */
    public override func internal_load_model(path: String = "",
                                        contextParams: ModelAndContextParams = .default) throws -> Bool
    {
        ...
        // context_params does not have seed
        ...
        if self.contextParams.custom_prompt_format.isEmpty {
            // Get model's chat template
            let modelChatTemplate = self.load_chat_template() ?? """
    {% for message in messages %}{% if loop.first and messages[0]['role'] != 'system' %}{{ '<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n' }}{% endif %}{{ '<|im_start|>' + message['role'] + '\n' + message['content'] + '<|im_end|>\n' }}{% endfor %}{% if add_generation_prompt %}{{ '<|im_start|>assistant\n' }}{% endif %}
    """
            print("LLaMa.llm_load_model using model default jinja chat template: \n\(modelChatTemplate)")
            ...
            print("LLaMa.llm_load_model using custom chat template: \n\(self.contextParams.custom_prompt_format)")
            self.chatTemplate = ChatTemplate(
                source: self.contextParams.custom_prompt_format,
                bosToken: llm_bos_token_str ?? "<s>",
                eosToken: llm_eos_token_str ?? "</s>"
            )
        }
    ...
    private func init_sampling_param(seed: Int = 0) {
...
//            temporary_invalid_cchars.removeLast(validSuffixLength)
     ...
     */
    public override func LLMTokenize(_ input: String, add_bos: Bool? = nil,
                                     ...
                                     use_template: Bool = true) -> [ModelToken]
    {
        ...
        let n_tokens   = Int32(utf8_count) + (final_add_bos ? 1 : 0) + 1
        ...
        // First checks if the model has an encoder component. Based on the source code,
        // this is true for T5 and T5Encoder architectures.
    ...
    public override func load_state() {
           ...
           FileManager.default.fileExists(atPath: self.contextParams.state_dump_path)
        {
            ...
            print("LLaMa.load_state(): state dump is disabled")
        }
    }
