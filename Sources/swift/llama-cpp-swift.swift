...
public actor LLama {
  ...
  private func tokenize(text: String, add_bos: Bool) -> [llama_token] {
    ...
    let tokenCount = llama_tokenize(
      model.vocab, text, Int32(utf8Data.count), tokens, Int32(nTokens), add_bos, false)
  ...
  private func tokenToPieceArray(token: llama_token) -> [CChar] {
    ...
    //change model.model to model.vocab wangqi
    var nTokens = llama_token_to_piece(model.vocab, token, &buffer, 8, 0, false)

    if nTokens < 0 {
      let requiredSize = -nTokens
      buffer = [CChar](repeating: 0, count: Int(requiredSize))
      //change model.model to model.vocab wangqi
      nTokens = llama_token_to_piece(model.vocab, token, &buffer, requiredSize, 0, false)
    }
